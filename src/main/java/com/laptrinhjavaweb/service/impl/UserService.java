...
public class UserService implements IUserService {
    ...
    @Override
    public List<StaffResposeDTO> getStaffByBuilding(Long buildingId) {
        List<StaffResposeDTO> result = new ArrayList<>();
        List<UserEntity> staffCheckeds = userRepository.getUserByBuildingId(buildingId);
        List<UserEntity> staffUnCheckeds = userRepository.findByStatusAndRoles_Code(1, "STAFF");
        /*for(UserEntity item : staffCheckeds){
            result.add(converToStaffResposeDTO(item, true));
        }*/
        for(UserEntity item : staffUnCheckeds){
            if(staffCheckeds.contains(item)){
                result.add(converToStaffResposeDTO(item, true));
            }
            else{
                result.add(converToStaffResposeDTO(item, false));
            }
        }
        return result;
    }

    private StaffResposeDTO converToStaffResposeDTO(UserEntity userEntity, boolean checked) {
        StaffResposeDTO result = new StaffResposeDTO();
